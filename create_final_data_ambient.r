...
query_select <- paste0("select * from nz_ambient.dex_dmf WHERE characteristic_short_description LIKE 'AAC_StoreType' AND characteristic_value_long_description LIKE 'SUPER' ")
...
write.table(super_shops_ids, "/home/michela/nz_ambient/super_shops_ids.psv", sep = "|", row.names = FALSE, col.names = TRUE)
...
write.table(dex_cau, "/home/michela/nz_ambient/dex_cau.psv", sep = "|", row.names = FALSE, col.names = TRUE)
...
sample_of_stores_itens <- major_minor_only[sample(nrow(major_minor_only), trunc(nrow(major_minor_only)*0.0015)), ]
...
write.table(sales_by_store, "/home/michela/nz_ambient/sales_by_store.psv", sep = "|", row.names = FALSE, col.names = TRUE)
...
top_5 <- sales_by_store_59_weeks_2[, tail(.SD, 5)]
...
write.table(final_list_shops_itens, "/home/michela/nz_ambient/final_list_shops_itens.psv", sep = "|", row.names = FALSE, col.names = TRUE)
...
write.table(final_list_shops_itens_small, "/home/michela/nz_ambient/final_list_shops_itens_small.psv", sep = "|", row.names = FALSE, col.names = TRUE)
...
write.table(final_list_shops_itens_small_short, "/home/michela/nz_ambient/final_list_shops_itens_small_short.psv", sep = "|", row.names = FALSE, col.names = TRUE)
write.table(final_list_shops_itens_small_long, "/home/michela/nz_ambient/final_list_shops_itens_small_long.psv", sep = "|", row.names = FALSE, col.names = TRUE)
...
final_list_shops_itens_small <- fread("/home/michela/nz_ambient/final_list_shops_itens_small.psv", sep = "|")
final_list_shops_itens_small_short <- fread("/home/michela/nz_ambient/final_list_shops_itens_small_short.psv", sep = "|")
final_list_shops_itens_small_long <- fread("/home/michela/nz_ambient/final_list_shops_itens_small_long.psv", sep = "|")
...
# The output is nz_ambient_dex_cau_simple
############################################################################################

cols_to_keep <- c("week_id","shop_id","item_id","new_causal_name")
nz_ambient_dex_cau_simple <- dex_cau_selected_stores_items[,cols_to_keep, with=F] 

setkey(nz_ambient_dex_cau_simple)
nz_ambient_dex_cau_simple <- unique(nz_ambient_dex_cau_simple)
dim(nz_ambient_dex_cau_simple)
#[1] 6534    4

nz_ambient_dex_cau_simple <- nz_ambient_dex_cau_simple[!is.na(week_id)]
nz_ambient_dex_cau_simple <- nz_ambient_dex_cau_simple[!is.na(new_causal_name)]
dim(nz_ambient_dex_cau_simple)
#[1] 6534    4
...
nz_ambient_dex_dwh <- rbind(nz_ambient_dex_dwh, temp_1)}

dim(nz_ambient_dex_dwh)
#[1]32952    12
setkey(nz_ambient_dex_dwh)

nz_ambient_dex_dwh <- unique(nz_ambient_dex_dwh)
...
setkey(nz_ambient_dex_dwh, item_id)
setkey(product_description, item_id)

final_data_ambient <- product_description[nz_ambient_dex_dwh, allow.cartesian=TRUE]
dim(final_data_ambient)
#[1] 32952    13


setkeyv(final_data_ambient,c("item_id", "week_id","shop_id"))
setkeyv(nz_ambient_dex_cau_simple,c("item_id", "week_id","shop_id"))
final_data_ambient <- nz_ambient_dex_cau_simple[final_data_ambient]
dim(final_data_ambient)
#[1] 33978    14

setkeyv(final_data_ambient,c("item_id", "shop_id"))
setkeyv(final_list_shops_itens_small,c("item_id", "shop_id"))

final_data_ambient <- final_list_shops_itens_small[final_data_ambient]
final_data_ambient[is.na(new_causal_name), new_causal_name:= "No display"]

##########################################
# Add indicators of causal
##########################################

final_data_ambient$Major_Ind <- as.integer(grepl(pattern = "Major Display", x = final_data_ambient$new_causal_name))
final_data_ambient$Minor_Ind <- as.integer(grepl(pattern = "Minor Display", x = final_data_ambient$new_causal_name))
final_data_ambient$Catalogue_feature_Ind <- as.integer(grepl(pattern = "Catalogue or Feature", x = final_data_ambient$new_causal_name))
final_data_ambient$No_display_Ind <- as.integer(grepl(pattern = "No display", x = final_data_ambient$new_causal_name))
...
setkeyv(final_data_ambient, c("week_id","shop_id"))

final_data_ambient <- dex_dmf[final_data_ambient]

##########################################
# Save data
##########################################
write.table(final_data_ambient, "/home/michela/nz_ambient/final_data_ambient.psv", sep = "|", row.names = FALSE, col.names = TRUE)
