...
test_data_copy9  <- test_data_ambient_72

#test_data_copy9$shop_id <- as.integer(test_data_copy9$shop_id)
test_data_copy9$corrected_sales_in_volume <- as.numeric(test_data_copy9$corrected_sales_in_volume)
test_data_copy9$last_volume <- as.numeric(test_data_copy9$last_volume)
test_data_copy9$mode_volume <- as.numeric(test_data_copy9$mode_volume)

 preObj <- preProcess(test_data_copy9[, -c(1:11,17:28,49)], method=c("center", "scale"))
 newData <- predict(preObj, test_data_copy9[, -c(1:11,17:28,49)])
 test_data_copy9 <- cbind(newData, test_data_copy9[, c(1:11,17:28,49)])
...
training_data_copyv8a<- training_data_ambient_72

training_data_copyv8a$shop_id <- as.integer(training_data_copyv8a$shop_id)
training_data_copyv8a$corrected_sales_in_volume <- as.numeric(training_data_copyv8a$corrected_sales_in_volume)
training_data_copyv8a$last_volume <- as.numeric(training_data_copyv8a$last_volume)
training_data_copyv8a$mode_volume <- as.numeric(training_data_copyv8a$mode_volume)

 preObj <- preProcess(training_data_copyv6[, -c(1:11,17:28,49)], method=c("center", "scale"))
 newData <- predict(preObj, training_data_copyv6[, -c(1:12,18:25,47:48,51)])
 training_data_copyv6 <- cbind(newData, training_data_copyv6[, c(1:12,18:25,47:48,51)])
