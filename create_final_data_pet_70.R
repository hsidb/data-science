...
######################################################
dbExistsTable(con, c("nz_pet_food", "dex_dwh"))
...
#query_select <- paste0("select * from nz_pet_food.dex_dmf WHERE characteristic_short_description LIKE 'AAC_StoreType' AND characteristic_value_long_description LIKE 'SUPER' ")
...
write.table(super_shops_ids, "/home/hagen/nz_pet_food/super_shops_ids.psv", sep = "|", row.names = FALSE, col.names = TRUE)

######################################################
# Causal info from selected stores
######################################################

schema_cat <- "nz_pet_food"
...
write.table(dex_cau, "/home/hagen/nz_pet_food/dex_cau.psv", sep = "|", row.names = FALSE, col.names = TRUE)
...
the_query <- paste0("select shop_id, item_id, min(week_id), max(week_id), sum(corrected_sales_in_volume) from nz_pet_food.dex_dwh where shop_id in (",super_shops_ids,") group by shop_id, item_id")
...
write.table(sales_by_store, "/home/hagen/nz_pet_food/sales_by_store.psv", sep = "|", row.names = FALSE, col.names = TRUE)
...
#########################################################################
 ...
 sales_by_store_155_weeks <- sales_by_store[ total_weeks == 155]
 setkeyv(sales_by_store_155_weeks, c("shop_id","sum_corrected_sales_in_volume"))
# 
 dim(sales_by_store)
# #[1] 330530      7
 dim(sales_by_store_155_weeks)
# #[1] 119804      7
# 
# 119804/330530
# #[1] 0.3624603
# 
# #bottom_5 <- sales_by_store_155_weeks[, head(.SD, 5), by=shop_id]
# #top_5 <- sales_by_store_155_weeks[, tail(.SD, 5), by=shop_id]
 top_25 <- sales_by_store_155_weeks[, tail(.SD, 25), by=shop_id]

 
 #alternatively:
 
  pet_desc <- read_delim("~/Desktop/nielsen/pet_desc.csv",

 library(readr)
 
 
 pet_food_market_share <- read_delim("~/Desktop/nielsen/new market share pet.csv","\t", escape_double = FALSE, trim_ws = TRUE)

 View(pet_food_market_share)
 top_25_2 <- pet_food_market_share
 
 
# count_itens <- sales_by_store_155_weeks[,.N , by=shop_id]
...
# setkeyv(sales_by_store_155_weeks, c("shop_id","sum_corrected_sales_in_volume"))
# sales_by_store_155_weeks[, value := sequence(.N), by = shop_id]
# 
# setkeyv(sales_by_store_155_weeks, c("shop_id","value"))
# setkeyv(count_itens, c("shop_id","value"))
# 
# middle_5 <- sales_by_store_155_weeks[count_itens]
...
# sales_by_store_155_weeks_2 <- sales_by_store_155_weeks[,sum_sales_all_itens := sum(sum_corrected_sales_in_volume), by = shop_id]
# cols_to_keep <- c("shop_id","sum_sales_all_itens")
# sales_by_store_155_weeks_2 <- sales_by_store_155_weeks_2[,cols_to_keep, with=FALSE]
# 
# sales_by_store_155_weeks_2 <- unique(sales_by_store_155_weeks_2)
# setkey(sales_by_store_155_weeks_2,sum_sales_all_itens)
# 
# #bottom_5 <- sales_by_store_155_weeks_2[, head(.SD, 5)]
# top_25 <- sales_by_store_155_weeks_2[, tail(.SD, 25)]
# 
# 
# dim(sales_by_store_155_weeks_2)
# #[1] 345   2

# the_middle <- as.integer(trunc(nrow(sales_by_store_155_weeks_2)/2))
# 
# middle_5 <- rbind(sales_by_store_155_weeks_2[the_middle], sales_by_store_155_weeks_2[the_middle+1], sales_by_store_155_weeks_2[the_middle+2], sales_by_store_155_weeks_2[the_middle-1], sales_by_store_155_weeks_2[the_middle-2])
# 
...
write.table(final_list_shops_itens, "/home/hagen/nz_pet_food/final_list_shops_itens.psv", sep = "|", row.names = FALSE, col.names = TRUE)
...
write.table(final_list_shops_itens_small, "/home/hagen/nz_pet_food/final_list_shops_itens_small.psv", sep = "|", row.names = FALSE, col.names = TRUE)
...
write.table(final_list_shops_itens_small_short, "/home/hagen/nz_pet_food/final_list_shops_itens_small_short.psv", sep = "|", row.names = FALSE, col.names = TRUE)
write.table(final_list_shops_itens_small_long, "/home/hagen/nz_pet_food/final_list_shops_itens_small_long.psv", sep = "|", row.names = FALSE, col.names = TRUE)
...
final_list_shops_itens <- fread("/home/hagen/nz_pet_food/final_list_shops_itens_small.psv", sep = "|")
final_list_shops_itens_small <- fread("/home/hagen/nz_pet_food/final_list_shops_itens_small.psv", sep = "|")
final_list_shops_itens_small_short <- fread("/home/hagen/nz_pet_food/final_list_shops_itens_small_short.psv", sep = "|")
final_list_shops_itens_small_long <- fread("/home/hagen/nz_pet_food/final_list_shops_itens_small_long.psv", sep = "|")
dex_cau <- fread("/home/hagen/nz_pet_food/dex_cau.psv", sep = "|")
super_shops_ids<- fread("/home/hagen/nz_pet_food/super_shops_ids.psv", sep = "|")
...
# The output is nz_pet_food_dex_cau_simple
############################################################################################

cols_to_keep <- c("week_id","shop_id","item_id","new_causal_name")
nz_pet_food_dex_cau_simple <- dex_cau_selected_stores_items[,cols_to_keep, with=F] 

setkey(nz_pet_food_dex_cau_simple)
nz_pet_food_dex_cau_simple <- unique(nz_pet_food_dex_cau_simple)
dim(nz_pet_food_dex_cau_simple)
#[[1] 10429     4
nz_pet_food_dex_cau_simple <- nz_pet_food_dex_cau_simple[,new_causal_name := paste(new_causal_name,collapse=","),by=c("week_id","shop_id","item_id")]

nz_pet_food_dex_cau_simple <- nz_pet_food_dex_cau_simple[!is.na(week_id)]
nz_pet_food_dex_cau_simple <- nz_pet_food_dex_cau_simple[!is.na(new_causal_name)]
dim(nz_pet_food_dex_cau_simple)
#[1] 10429     4
...
nz_pet_food_dex_dwh <- data.table()
for(i in 1:nrow(final_list_shops_itens_small_short)) {
  the_query <- paste0("select * from nz_pet_food.dex_dwh where shop_id = ",final_list_shops_itens_small_short$shop_id[i], " AND item_id in (",final_list_shops_itens_small_short$list_of_itens[i]," )"  )
  ...
  nz_pet_food_dex_dwh <- rbind(nz_pet_food_dex_dwh, temp_1)}

dim(nz_pet_food_dex_dwh)
#[1]32952    12
setkey(nz_pet_food_dex_dwh)

nz_pet_food_dex_dwh <- unique(nz_pet_food_dex_dwh)
...
for(i in 1:nrow(final_list_shops_itens_small_short)) {
  the_query <- paste0("select * from nz_pet_food.dex_dim where item_id in ( ",final_list_shops_itens_small_short$list_of_itens[i]," )"  )
  temp_1 <- dbGetQuery(con, the_query)
...
setkey(nz_pet_food_dex_dwh, item_id)
setkey(product_description, item_id)

final_data_pet_food <- product_description[nz_pet_food_dex_dwh, allow.cartesian=TRUE]
dim(final_data_pet_food)
#[1] 72867    13


setkeyv(final_data_pet_food,c("item_id", "week_id","shop_id"))
setkeyv(nz_pet_food_dex_cau_simple,c("item_id", "week_id","shop_id"))
final_data_pet_food <- nz_pet_food_dex_cau_simple[final_data_pet_food]
dim(final_data_pet_food)
#[1] 73822    14

setkeyv(final_data_pet_food,c("item_id", "shop_id"))
setkeyv(final_list_shops_itens_small,c("item_id", "shop_id"))

final_data_pet_food <- final_list_shops_itens_small[final_data_pet_food]
final_data_pet_food[is.na(new_causal_name), new_causal_name:= "No display"]

##########################################
# Add indicators of causal
##########################################

final_data_pet_food$Major_Ind <- as.integer(grepl(pattern = "Major Display", x = final_data_pet_food$new_causal_name))
final_data_pet_food$Minor_Ind <- as.integer(grepl(pattern = "Minor Display", x = final_data_pet_food$new_causal_name))
final_data_pet_food$Catalogue_feature_Ind <- as.integer(grepl(pattern = "Catalogue or Feature", x = final_data_pet_food$new_causal_name))
final_data_pet_food$No_display_Ind <- as.integer(grepl(pattern = "No display", x = final_data_pet_food$new_causal_name))
...
query_2 <- paste0("select * from nz_pet_food.dex_dmf WHERE characteristic_short_description = 'AAC_Retailer' AND shop_id in (",super_shops_ids,")" )
...
setkeyv(final_data_pet_food, c("week_id","shop_id"))

final_data_pet_food <- dex_dmf[final_data_pet_food]

##########################################
# Save data
##########################################
write.table(final_data_pet_food, "/home/hagen/nz_pet_food/final_data_pet_70.psv", sep = "|", row.names = FALSE, col.names = TRUE)